...
> If (_m_ * q) <= _n_ (a multiple of _m_ is less than _n_), it's implies that there's a natural number r (remainder) such that _n_ = (_m_ * q) + r (with r always less then _m_).<br>
